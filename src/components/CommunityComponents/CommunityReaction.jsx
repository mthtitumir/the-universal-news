    ...
    return (
        <>
            <div className="flex justify-evenly items-center mt-2"> {/*bottom like comment share nav*/}
                <h3 onClick={()=>handleLike(id, "like")} className={iconStyles}><AiOutlineHeart className="hover:rounded-full" /> {likes}</h3>
                <h3 onClick={()=>handleLike(id, "dislike")} className={iconStyles}><RiDislikeLine className="hover:rounded-full" /> {dislikes}</h3>
                <h3 className={iconStyles}><BiCommentDetail className="hover:rounded-full" /> {comments?.length}</h3>
